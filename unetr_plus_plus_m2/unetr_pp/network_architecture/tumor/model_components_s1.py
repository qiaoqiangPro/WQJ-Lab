...
from unetr_pp.network_architecture.dynunet_block import NewEmbedding1,NewEmbedding4
import torch
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
        ...
        stem_layer = nn.Sequential(
            # NewEmbedding4(in_channels=in_channels, out_channels=dims[0]),
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x):
        ...
        S, H, W = x.shape[-3],x.shape[-3],x.shape[-2]
...
class UnetrUpBlock(nn.Module):
    def     __init__(
        ...
        self.skin_attention_1 = SwinTransformerBlock_kv(
                    dim=embed_dim,
                    input_resolution=input_resolution,
                    num_heads=num_heads,
                    window_size=window_size,
    ...
    def forward(self, inp, skip):
        ...
        x = self.skin_attention_1(x, attn_mask, skip=skip, x_up=x_up)
