...
model = monai.networks.nets.ViT(spatial_dims=3,in_channels=1, img_size=(256,256,24),patch_size=(16,16,16) ,num_classes=2 ,pos_embed='conv', classification=True).to(device)
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        for val_data in val_loader:
            ...
            with torch.no_grad():
                ...
                value = torch.eq(val_outputs[0].argmax(dim=1), val_labels.argmax(dim=1))
...
# create a validation data loader
test_transforms = Compose([ScaleIntensity(), EnsureChannelFirst(), Resize((512, 512, 32))])
